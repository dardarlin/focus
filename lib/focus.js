(function($){
	...
	*/
	...
	function disableFocus(selector){
		if($(selector+'.[focusable]').length > 0){
			$(selector).removeAttr('focusable');
			buildFocusTree();
			console.log(tree);
		}
	}
	function enableFocus(selector){
		if($(selector+'.[focusable]').length == 0){
			$(selector).attr('focusable','');
			buildFocusTree();
		}
	}
	function outFn(){
