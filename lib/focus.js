(function($){
	...
	function buildFocusDomTree(){
		var defLevels = $("[focus-level]");
		if(!defLevels){
		}else{
			for (var i = 0; i < defLevels.length; i++) { 
				var level = defLevels[i];
				var levelName = level.attributes['focus-level'].value;
				var levelArr = [];
				var defGroups = $(level).find("[focus-group]");
				var groupArr = []
				for (var j = 0; j < defGroups.length; j++) {
					var group = defGroups[j];
					var groupName = group.attributes['focus-group'].value;
					var elements = [];
	...
	*/
	...
	function findParent(element,back){
		...
		for (var i = 0; i < parents.length; i++) {
			...
			if($(parent).attr("focus-group") && !back.level){
				back.group = parent.attributes['focus-group'].value;
			}
			if($(parent).attr("focus-level")){
				if(!back.group){
					back.group = 'group0';
				}
				back.level = parent.attributes['focus-level'].value;
				return back;
			}
		};
	}
	...
	function findNearbyNext(curFocus,back,directive){
		...
		next = findNearestElements(curFocus,elements,directive);
